...
# ~~~
# Description:
# This is the main file for the numerical experiment in section 4.6: the case study of the error estimator.
...
TR_IRGNM_setup = {'Inner_IRGNM_setup': Inner_IRGNM_setupQrVr,
                 ...
                 'radius': init_radius,                                                   # init trust region radius
                 'shrink_factor': .5,                                
                 'miniter':0, 
                 'maxiter':k_max, 
...
# Setup for Qr-Vr TR IRGNM (unassembled)
TR_IRGNM_setup_unassembled = {'Inner_IRGNM_setup': Inner_IRGNM_setupQrVr,
                 'tau': tau,                                                    # discrepancy parameter
                 'noise_level': noise_level,                                    # moise level
                 'theta': theta,                                                # lower bound for the choice of the regularization
                 'Theta': Theta,                                                # upper bound for the choice of the regularization
                 ...
                 'return_stages': False, 
                 'return_subproblem_stages': False,
                 'reductor_type': 'non_assembled',                                ### changed here
                 'bounds': bounds,
                 'display_iteration_process': 1,
                 'outer_regularization': False,                                 # use outer regularization
                 'alpha_init': alpha0,                                          # initial alpha for first inner IRGNM or outer regularization
...
if 'Qr-Vr TR IRGNM' in opt_methods:
    ...
    # assembled strategy
    q_Qr_Vr_off_on, history_QTRRB_off_on = Qr_Vr_TR_IRGNM(parameter_space, fom_IP, TR_IRGNM_setup, q0, tol, skip_error_est_assembly_condition = False)  
    # unassembled strategy
    q_Qr_Vr_unassemled, history_QTRRB_unassembled = Qr_Vr_TR_IRGNM(parameter_space, fom_IP, TR_IRGNM_setup_unassembled, q0, tol, skip_error_est_assembly_condition = False)  
...
if 'q_Qr_Vr' in locals() and 1:
     plt.figure()
     plt.plot(history_QTRRB['fom_solves_for_assembling_residual_error_est'],'.-', label = 'Offline/online')
     plt.plot(history_QTRRB['fom_solves_for_unassembled_residual_error_est'], '.-', label = 'Online')
     plt.plot(history_QTRRB['true_fom_solves_for_error_est'],'.-', label = 'Mixed')
     plt.legend()
     plt.xlabel("k")
...
if plot_parameters:
   ...
   fom_IP.plot_matplotlib(fom_IP.opt_data['u_noise'].to_numpy()[0], 'Noisy data', save_plots, problem_type+f'_N={N}_noisydata.png', save_path)
   fom_IP.plot_matplotlib(fom_IP.opt_data['noise'], 'Noise', save_plots, problem_type+f'_N={N}_noise.png', save_path)
...
if "FOM IRGNM" in opt_methods and 'Qr IRGNM' in opt_methods and 'Qr-Vr TR IRGNM' in opt_methods: 
    ...
    plt.figure()
    fom_IP.plot_matplotlib(abs((q_FOM-q_Qr)), r'abs $d^{Q_r}$', save_plots, problem_type+f'_N={N}_rel_diff2_fom-qfom.png' , save_path )
    plt.figure()
    fom_IP.plot_matplotlib(abs((q_Qr-q_Qr_Vr)), r'abs $d^{Q_r,Q_r-V_r}$', save_plots, problem_type+f'_N={N}_rel_diff2_qfom-qtrrb.png', save_path  )
    plt.figure()
    fom_IP.plot_matplotlib(abs((q_FOM-q_Qr_Vr)), r'abs $d^{Q_r-V_r}$', save_plots, problem_type+f'_N={N}_rel_diff2_fom-qtrrb.png', save_path  )
...
if 'q_Qr' in locals():
    pass
if 'q_Qr_Vr' in locals():
    plt.semilogy(history_QTRRB_off_on["output_res"], '-o', label= r'assembled')
    plt.semilogy(history_QTRRB["output_res"], '-o', label= r'$Q_r$-$V_r$ IRGNM')
    plt.semilogy(history_QTRRB_unassembled["output_res"], '-o', label= 'unassembled')
if 'q_FOM' in locals():
    pass
...
if 'q_Qr' in locals():
    pass
if 'q_Qr_Vr' in locals():
    plt.semilogy(history_QTRRB_off_on["time_steps"], history_QTRRB_off_on["output_res"], '-o', label= r'Offline/online')
    plt.semilogy(history_QTRRB_unassembled["time_steps"], history_QTRRB_unassembled["output_res"], '-o', label= 'Online')
    plt.semilogy(history_QTRRB["time_steps"], history_QTRRB["output_res"], '-o', label= r'Mixed')
if 'q_FOM' in locals():
   pass
value = tol+noise_level*tau
plt.axhline(y=value, color='g', linestyle='-', label = r'$\tau\delta$')
